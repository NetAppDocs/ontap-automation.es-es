...
Algunas de las llamadas a la API REST, como las que usan POST o PATCH, requieren una entrada JSON para el cuerpo de la solicitud. Los ejemplos de entrada JSON se presentan por separado de los ejemplos de rizo para mayor claridad. Puede utilizar los ejemplos de entrada JSON con una de las técnicas descritas a continuación.

.Guardar en archivo local
Puede copiar el ejemplo JSON en un archivo y guardarlo localmente. El comando cURL hace referencia al archivo que utiliza `--data` parámetro con el valor que indica el nombre de archivo con a. `@` prefijo.

.Pegue en el terminal después del ejemplo de cURL
Primero debe copiar y pegar el ejemplo de cURL en el shell de terminal. A continuación, edite el ejemplo para eliminar completamente el `--data` parámetro al final y sustitúyalo por el `--data-raw` parámetro. Por último, copie y pegue en el ejemplo JSON para que siga el comando cURL y el parámetro actualizado.
