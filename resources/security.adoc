...
.Habilitación de anti-ransomware
Puedes controlar el funcionamiento de la función de habilitación de Autonomous Ransomware Protection (ARP). Esto incluye la recuperación y modificación de los ajustes de configuración. Este tipo de recurso se introdujo con ONTAP 9.18.1.
...
.seguridad de la red del clúster
Puede recuperar y actualizar la configuración de seguridad de la red del clúster, incluidos los certificados. Este tipo de recurso se introdujo con ONTAP 9.18.
...
.Seguridad de red de alta disponibilidad
Puede recuperar y actualizar la configuración de seguridad de la red HA. Este tipo de recurso se introdujo con ONTAP 9.18.
...
.Directivas IPSec
Puedes utilizar este conjunto de llamadas a la API para gestionar las políticas vigentes en una implementación de IPSec. Este tipo de recurso es nuevo en ONTAP 9.8. Se han actualizado con la 9.19 para recuperar las políticas utilizadas en la red del clúster.

.Asociaciones de seguridad IPSec
Puedes usar este conjunto de llamadas a la API para gestionar las asociaciones de seguridad vigentes en una implementación de IPSec. Este tipo de recurso es nuevo con ONTAP 9.8. Se actualizaron con la 9.19 para recuperar las asociaciones de seguridad usadas para la red del clúster.
...
.Autenticación web
La autenticación web (WebAuthn) es un estándar web para autenticar de forma segura a los usuarios basándose en la criptografía de clave pública. Con ONTAP, admite la administración de MFAs resistentes al phishing a través de System Manager y la API de REST de ONTAP. Esta función se añadió con ONTAP 9.16 y se actualizó con ONTAP 9.19.

.Whoami
Existe un endpoint que permite mostrar los detalles del usuario autenticado, incluyendo el nombre de usuario, el rol y los permisos. Este tipo de recurso se introdujo con ONTAP 9.19.
