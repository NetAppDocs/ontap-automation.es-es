...
.Autenticación web
La autenticación web (WebAuthn) es un estándar web para autenticar usuarios de forma segura basado en criptografía de clave pública. Con ONTAP, admite la administración de MFAs resistentes al phishing a través de System Manager y la API de REST de ONTAP. Esta función se añadió con ONTAP 9.16 y se actualizó con ONTAP 9.19.1.

.Whoami
Se dispone de un endpoint para mostrar los detalles del usuario autenticado, incluyendo nombre de usuario, rol y permisos. Este tipo de recurso se introdujo con ONTAP 9.19.1.
