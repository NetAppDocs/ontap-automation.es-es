...
La versión 9.19.1 de ONTAP incluye varias actualizaciones que siguen mejorando y perfeccionando la API de REST de ONTAP.

Seguridad:: Se ha añadido una nueva  `whoami` función para recuperar la información sobre el nombre de usuario, el rol y los permisos del usuario que ha iniciado sesión. También se ha mejorado la autenticación web (WebAuthn). IPsec se ha actualizado para recuperar las políticas y las asociaciones de seguridad utilizadas con la red del clúster.
SAN:: La administración de mapas LUN se ha actualizado con una capacidad de actualización mejorada.
...
La versión 9.18.1 de ONTAP sigue ampliando las capacidades de la API de REST de ONTAP con trece nuevas llamadas a la API. Las mejoras se centran principalmente en la seguridad, pero también incluyen una mejora relacionada con la administración de almacenamiento.

Seguridad:: Se han introducido varias mejoras de seguridad con la API de REST de ONTAP 9.18.1. Puedes usar los nuevos puntos finales de red de clúster para mostrar y actualizar la configuración de seguridad de la red de clúster, incluida la gestión de certificados. Además, puedes administrar la configuración de seguridad de la red de HA para el tráfico NVLog. La protección autónoma contra ransomware (ARP) se ha mejorado para admitir la visualización y actualización de la configuración de habilitación automática.
